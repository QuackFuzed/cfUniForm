...
<cfscript>
	...
	inlinePlacement = getBaseTagData('cf_form').attributes.errorMessageInlinePlacement;
	...
	*/
	...
	attributes.timestampSetups = structNew();
...
<fieldset class="#attributes.class#" id="#attributes.id#"></cfoutput>
	...
	<cfoutput><div class="cfu-fs-inner-wrap">#attributes.content#</cfoutput>
	
<cfset formTagData = getBaseTagData("cf_form")>
...
<cfloop from="1" to="#arrayLen(thisTag.fields)#" index="f">
	...
	<cfset thisFieldRequired = structKeyExists(requiredFields, thisField.name) OR thisField.isRequired>

	<cfif thisField.isDisabled AND thisField.type eq "radio">
		<cfset thisField.type = "text">
		<cfset thisField.value = findReadOnlyValue( thisField )>
	</cfif>
	...
	<cfif (isSimpleValue(thisField.pluginSetup) AND len(trim(thisField.pluginSetup)) GT 0) OR (isStruct(thisField.pluginSetup) AND structCount(thisField.pluginSetup) GT 0)>
		<cfscript>
			switch ( thisField.type ) {
				...
				case "timestamp" : {
					structInsert(attributes.timestampSetups, thisField.id, thisField.pluginSetup);
					break;
				}
			}
		</cfscript>
	...
	<cfelseif (fieldHasErrors(thisField.name, errors)) AND (listFindNoCase("inline,both", placement) GT 0) AND inlinePlacement eq "top">
		<cfoutput>#outputInlineErrors(errors,thisField)#</cfoutput>
	<cfelseif (fieldHasErrors(thisField.name, errors)) AND inlinePlacement eq "bottom">
		<cfoutput>
			<div class="ctrlHolder error<cfif structKeyExists(thisField, 'containerClass') AND len(thisField.containerClass) GT 0> #thisField.containerClass#</cfif><cfif thisField.type IS 'checkbox'> noLabel</cfif>">
		</cfoutput>
		<cfelse>
	</cfif>
	
	<cfif (listFindNoCase("checkboxgroup,checkbox,radio,custom,rating", thisField.type) EQ 0)>
		<cfoutput><label for="#thisField.id#" class="#IIF( thisFieldRequired, DE('requiredField'), DE(''))#"><cfif thisFieldRequired><em>*</em></cfif> #thisField.label#</label></cfoutput>
	</cfif>
				...
				<cfif structKeyExists(thisField, "options")>
					<cfloop from="1" to="#arrayLen(thisField.options)#" index="o">
						<cfif NOT structKeyExists(thisField.options[o], "content")>
							<cfoutput><option value="#thisField.options[o].value#"<cfif thisField.options[o].isSelected> selected="selected"</cfif> class="#thisField.options[o].optionClass#">#thisField.options[o].display#</option></cfoutput>
						<cfelse>
				...
				<cfelseif structKeyExists(thisField, "optiongroups")>
					<cfloop from="1" to="#arrayLen(thisField.optiongroups)#" index="og">
						<cfif NOT structKeyExists(thisField.optiongroups[og], "content")>
							<cfoutput><optgroup label="#thisField.optiongroups[og].label#"<cfif thisField.isDisabled> disabled="disabled"</cfif> <cfif thisField.optiongroups[og].value neq ""> value="#thisField.optiongroups[og].value#"</cfif>></cfoutput>
							<cfif structKeyExists(thisField.optiongroups[og], "options")>
								<cfloop from="1" to="#arrayLen(thisField.optiongroups[og].options)#" index="o">
									<cfif NOT structKeyExists(thisField.optiongroups[og].options[o], "content")>
										<cfoutput><option value="#thisField.optiongroups[og].options[o].value#"<cfif thisField.optiongroups[og].options[o].isSelected> selected="selected"</cfif>  class="#thisField.optiongroups[og].options[o].optionClass#">#thisField.optiongroups[og].options[o].display#</option></cfoutput>
									<cfelse>
		...
		<cfelseif thisField.type IS "radio">
			<cfoutput>
				<p class="label #IIF(thisFieldRequired, DE('requiredField'), DE(''))#">
					<cfif thisFieldRequired><em>*</em></cfif> #thisField.label#
				</p>
				<ul class="blockLabels">
			</cfoutput>
			<cfloop from="1" to="#arrayLen(thisField.radios)#" index="r">
				<cfoutput>
					<li>
						<label for="#thisField.id#-#r#" class="inlineLabel">

						<cfif thisField.isDisabled>
						<cfoutput>
						#thisField.radios[r].value#
						</cfoutput>
						<cfelse>
							<input name="#thisField.name#" id="#thisField.id#-#r#" value="#thisField.radios[r].value#"
								<cfif thisField.radios[r].isChecked> checked="checked"</cfif>
								type="radio"
								class="<cfif structKeyExists(thisField, 'inputClass') AND len(thisField.inputClass) GT 0> #thisField.inputClass#</cfif>
								<cfif (r EQ 1) AND (structKeyExists(requiredFields, thisField.name)) OR (thisField.isRequired)> required</cfif>"
							/>&nbsp;#thisField.radios[r].label#
						</cfif>
						</label>
					</li>
				</cfoutput>
		...
		<cfelseif thisField.type IS "checkboxgroup">
			<cfoutput>
				<p class="label #IIF(thisFieldRequired, DE('requiredField'), DE(''))#"><cfif thisFieldRequired><em>*</em></cfif> #thisField.label#</p>
				<ul class="blockLabels">
			</cfoutput>
			<cfif structKeyExists(thisField,"checkboxes")>
				<cfloop from="1" to="#arrayLen(thisField.checkboxes)#" index="c">
					<cfif len(thisField.checkboxes[c].id) EQ 0>
						<cfoutput>
						<li>
						<label for="#thisField.id#-#c#" class="inlineLabel #IIF(thisField.isDisabled, DE('disabledField'), DE(''))#">
							<input name="#thisField.name#" id="#thisField.id#-#c#" value="#thisField.checkboxes[c].value#"
								<cfif thisField.checkboxes[c].isChecked> checked="checked"</cfif>
								type="checkbox"
								class="checkbox<cfif structKeyExists(thisField, 'inputClass') AND len(thisField.inputClass) GT 0> #thisField.inputClass#</cfif>
								<cfif (c EQ 1) AND (structKeyExists(requiredFields, thisField.name)) OR (thisField.isRequired)> required</cfif>"
								<cfif thisField.isDisabled> disabled="disabled"</cfif>
							/>&nbsp;#thisField.checkboxes[c].label#
						</label>
						</li></cfoutput>
					<cfelse>
			...
			</cfif><cfoutput></ul></cfoutput>
		<cfelseif thisField.type IS "checkbox">
			<cfoutput>
			<ul>
				<li>
				<label for="#thisField.id#" class="checkboxLabel  #IIF(thisFieldRequired, DE('requiredField'), DE(''))# #IIF(thisField.isDisabled, DE('disabledField'), DE(''))#">
					<input name="#thisField.name#" id="#thisField.id#" value="#thisField.value#"
						type="checkbox"
						class="<cfif structKeyExists(thisField, 'inputClass') AND len(thisField.inputClass) GT 0> #thisField.inputClass#</cfif><cfif thisFieldRequired> required</cfif>"
						<cfif structKeyExists(thisField, "isChecked") AND thisField.isChecked> checked="checked"</cfif>
						<cfif thisField.isDisabled> disabled="disabled"</cfif>
					/>
				<cfif thisFieldRequired><em>*</em></cfif>&nbsp;#thisField.label#
				</label>
		...
		<cfelseif thisField.type is "plaintext">
			<cfoutput>
			<span class="plaintextInput" id="#thisField.id#">
				<cfif structKeyExists( thisField, "content" ) and len(thisField.content)>
				#thisField.content#
				<cfelse>
				#thisField.value#
				</cfif>
		...
		<!--- END: fieldType check --->
		<cfif len(thisField.beforeHint) GT 0><cfoutput>#thisField.beforeHint#</cfoutput></cfif>
		<cfif len(thisField.hint) GT 0><cfoutput><p class="formHint">#thisField.hint#</p></cfoutput></cfif>
		<cfif len(thisField.afterHint) GT 0><cfoutput>#thisField.afterHint#</cfoutput></cfif>
		
		<cfif fieldHasErrors(thisField.name,errors) AND inlinePlacement eq "bottom">
			<cfoutput>
				<div style="clear:both;display:block">
				#outputInlineErrors(errors,thisField)#
				</div>
			</cfoutput>
		</cfif>

		<!--- hello, I'm a little inconspicuous end div tag --->
		<cfoutput>
		</div>
		</cfoutput>
	...
	<cfscript>
		...
		if ( isStruct(errors) ) {
			...
			if ( arrayLen(errors) GT 0 AND structKeyExists(errors[1], "field") ) {
				_property = "field";
			} else if ( arrayLen(errors) GT 0 AND structKeyExists(errors[1], "clientfieldname") ) {
				_property = "clientfieldname";
			}
...
<cffunction name="outputInlineErrors" output="true" access="public" returntype="void" hint="">
	<cfargument name="errors" type="any" required="true"/>
	<cfargument name="thisField" type="any" required="true"/>
	<cfif isStruct(errors)>
		<cfoutput>
		<cfif inlinePlacement eq "top">
		<div class="ctrlHolder error<cfif structKeyExists(thisField, 'containerClass') AND len(thisField.containerClass) GT 0> #thisField.containerClass#</cfif><cfif thisField.type IS 'checkbox'> noLabel</cfif>">
		</cfif>
		<p id="error-#thisField.name#" class="errorField">#errors[thisField.name]#</p>
		</cfoutput>
	<cfelse>
		<cfscript>
			_usedIdx = "";
			...
			else if ( arrayLen(errors) GT 0 AND structKeyExists(errors[1],"propertyname") AND structKeyExists(errors[1],"property") EQ false )
			{
		...
		<cfloop from="1" to="#arrayLen(errors)#" index="e">
			<cfif (listFind(_usedIdx, e) EQ 0) AND (compareNoCase(errors[e][_property],thisField.name) EQ 0)>
				...
				<p id="error-#thisField.name#" class="errorField">#errors[e].message#</cfoutput>
...
<cffunction name="findReadOnlyValue" output="false" access="public" returntype="string" hint="finds the read-only value for a field">
	<cfargument name="thisField" type="any" required="true"/>
	<cfset var i = 1>
	<cfif structKeyExists(thisField, "readOnlyValue")>
		<cfreturn thisField.readOnlyValue>
	</cfif>

	<cfif structKeyExists(thisField, "radios") AND arrayLen(thisField.radios)>
		<cfloop from="1" to="#arrayLen(thisField.Radios)#" index="i">
			<cfif thisField.radios[i].isChecked>
				<cfreturn thisField.radios[i].readOnlyValue>
			</cfif>
		</cfloop>
	</cfif>
